...
── 散らかりの型 ──
  ● 深すぎる: 最大 7階層（2階層までが扱いやすい）
  ● 添付置き場が未設定: 283件が 27フォルダに散在

── ★ 移動で壊れるもの ──
  パス付きリンク: 616  ← 移動すると壊れるのはこれだけ
```
...
| | 内容 |
|---|---|
| **1) 安全な整理だけ**（おすすめ） | ルート直下のノートを受信箱へ、添付を集約、空フォルダは報告のみ |
| **2) 分類も整理** | 同じ種類のノートを1か所に寄せる |
| **3) 推奨構造に作り替える** | すべて分類して配置し直す |

**迷っていたら 1 を選んでください。** 後から 2 に進めますが、逆は戻せません。

> **この操作はこのキットで唯一、あなたのファイルを移動します。** そのため：
> - **移動する前に、何をどこへ動かすかを全部一覧で見せます**
> - 実行前に git のコミットかバックアップを取ります
> - **削除しません**（空フォルダも残します）。**ファイル名も変えません**
> - `[[フォルダ/名前]]` のようなパス付きリンクは、移動後に自動で書き換えます
> - 移動後に**ノート数とリンク切れ数を検証**し、合わなければ復旧を提案します
...
| `/vault-organize` | **散らかった vault を整理する。** 診断して、移動する前に必ず一覧で見せる |
